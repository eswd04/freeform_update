...
class FloatingView(
    ...
    private fun showAllAppsView() {
        if (Settings.canDrawOverlays(context)) {
            ...
            progressView=allAppsView?.findViewById(R.id.shwo_progress)
            ...
            val adapter = AllAppsAdapter(context, object : ClickListener {
                ...
                setLoadFinish(object :AllAppsAdapter.OnLoadFinish{
                    override fun loadFinish() {
                        GlobalScope.launch (Dispatchers.Main){
                            progressView?.animate()?.alpha(0F)?.setDuration(300)?.start()
                        }
                    }
                })
            }

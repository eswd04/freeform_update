...
class HookFramework : IXposedHookZygoteInit {
    ...
    private fun hookDMS() {
        val classLoader = Thread.currentThread().contextClassLoader
        val fuckClass = if (Build.VERSION.SDK_INT==Build.VERSION_CODES.Q||Build.VERSION.SDK_INT==Build.VERSION_CODES.R)XposedHelpers.findClass("com.android.server.wm.ActivityStackSupervisor",classLoader)
        else  XposedHelpers.findClass("com.android.server.wm.ActivityTaskSupervisor",classLoader)
        val record=XposedHelpers.findClass("android.content.pm.ActivityInfo",classLoader)
        XposedHelpers.findAndHookMethod(fuckClass,"isCallerAllowedToLaunchOnDisplay",Int::class.java,Int::class.java,Int::class.java,record,object :XC_MethodHook(){
            override fun afterHookedMethod(param: MethodHookParam?) { if (param != null) { param.result=true } }
        })
//        XposedHelpers.findAndHookMethod(fuckClass,"canPlaceEntityOnDisplay",Int::class.java,Int::class.java,Int::class.java,record,object :XC_MethodHook(){
//            override fun afterHookedMethod(param: MethodHookParam?) { if (param != null) { param.result=true } }
//        })
